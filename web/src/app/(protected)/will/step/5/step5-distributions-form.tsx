...
function allocationStatusForAsset(assetId: string, distributions: Distributions) {
  ...
  if (duplicates) {
    return {
      ...
      helper: "Each beneficiary should appear only once per asset.",
      total,
      duplicates,
    };
  }
...
function getRemainingPercent(allocs: Allocation[], index: number) {
  const others = allocs.reduce((sum, row, idx) => {
    if (idx === index) return sum;
    return sum + (Number.isFinite(row.percentage) ? row.percentage : 0);
  }, 0);

  return round2(100 - others);
}
...
export default function Step5DistributionsForm() {
  ...
  function allocateRemainingToRow(index: number) {
    if (!selectedAssetId) return;
    ...
    setDistributions((prev) => {
      ...
      arr[index] = {
        ...arr[index],
        percentage: getRemainingPercent(arr, index),
      };
      ...
      <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-1 text-xs leading-5 text-slate-500">
            Choose an asset and complete its distribution before moving to the next.
          </div>
        ...
        <div className="space-y-4">
          <div className={`rounded-[24px] border p-5 ${selectedTone.panel}`}>
          ...
          <div className="space-y-3">
            ...
            {selectedAllocs.map((row, idx) => {
              ...
              const remainderValue = getRemainingPercent(selectedAllocs, idx);
                ...
                <div
                  ...
                  className={
                    isDuplicate || invalidPercent
                      ? "rounded-[22px] border border-rose-200 bg-rose-50/60 p-4"
                      : "rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"
                  }
                >
                  <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_160px_120px] lg:items-end">
                    <div>
                      ...
                      {meta ? (
                        <div className="mt-3 rounded-2xl bg-[#f8fafc] px-3 py-3 text-xs leading-6 text-slate-600 ring-1 ring-slate-100">
                    ...
                    <div>
                      ...
                      <button
                        type="button"
                        onClick={() => allocateRemainingToRow(idx)}
                        className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-100"
                      >
                        Use remainder
                      </button>
                    </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700">
                      Suggested remainder: {remainderValue}%
                    </span>
          ...
          <div className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <ul className="mt-3 space-y-2 text-sm text-slate-600">
              ...
              <li>• Avoid duplicate beneficiary rows for the same asset.</li>
            </ul>
          </div>
        </div>
      </div>
